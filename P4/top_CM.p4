    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    Register<bit<32>, bit<16>>(32768) CM_1;//Regitser

    RegisterAction<bit<32>, bit<16>, bit<32>>(CM_1) CM_1_insert=//SALU
    {
        void apply(inout bit<32> register_data, out bit<32> result) 
        {
    ...
    Register<bit<32>, bit<16>>(32768) CM_2;//Regitser

    RegisterAction<bit<32>, bit<16>, bit<32>>(CM_2) CM_2_insert=//SALU
    {
        void apply(inout bit<32> register_data, out bit<32> result) 
        {
    ...
    Register<bit<32>, bit<16>>(32768) CM_3;//Regitser

    RegisterAction<bit<32>, bit<16>, bit<32>>(CM_3) CM_3_insert=//SALU
    {
        void apply(inout bit<32> register_data, out bit<32> result) 
        {
    ...
    action CM_1_a()
    {
        CM_1_insert.execute(hash_7.get({meta.output5_1}));
    }
    @stage(8) table CM_1_t {
        actions = { CM_1_a; }
        default_action = CM_1_a;
    }
    action CM_2_a()
    {
        CM_2_insert.execute(hash_8.get({meta.output5_1}));
    }
    @stage(8) table CM_2_t {
        actions = { CM_2_a; }
        default_action = CM_2_a;
    }
    action CM_3_a()
    {
        CM_3_insert.execute(hash_9.get({meta.output5_1}));
    }
    @stage(8) table CM_3_t {
        actions = { CM_3_a; }
        default_action = CM_3_a;
    }

    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
  ...
  Register<bit<32>, _>(65536, 0) totVotesReg1;
  
  Register<FreqId, _>(65536, {0, 0}) freqIdReg1;
  RegisterAction<bit<32>, _, bit<32>>(totVotesReg1) totVotesSalu1 = {
      void apply(inout bit<32> register_data, out bit<32> totVotes){
          register_data = register_data + 1;
          totVotes = register_data;
      }
  };

  RegisterAction<FreqId, _, bit<32>>(freqIdReg1) freqIdSalu1 = {
    void apply(inout FreqId register_data, out bit<32> register_id){
      if (meta.totVotes_div >= register_data.freq || meta.output5_1 == register_data.id){
        register_data.freq = register_data.freq + 1;
        register_data.id = meta.output5_1;
        register_id = register_data.id;
      }
    }
  };
  CRCPolynomial<bit<32>>(0x04C11DB7,true,false,false,0,0xFFFFFFFF) crc_32;
  Hash<bit<32>>(HashAlgorithm_t.CRC32, crc_32) hash1;
  action getHashAction1(){
    meta.hash_1 = hash1.get(meta.output5_1);  
  }
  table getHashTable1 {
    actions = {
      getHashAction1;
    }
    default_action = getHashAction1;
  }

  action totVotesAction1(){
    meta.totVotes = totVotesSalu1.execute(meta.hash_1[12:0]);
    meta.register_id = 0;
  }
  ...
  action voteDivAction() {
    meta.totVotes_div = meta.totVotes >> 5;
  }
  table voteDivTable1 {
    actions = {
      voteDivAction;
  ...
  action freqIdAction1() {
    meta.register_id = freqIdSalu1.execute(meta.hash_1[12:0]);
  }
  table freqIdTable1 {
    actions = {
      freqIdAction1;

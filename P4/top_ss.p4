    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
    ...
    Register<data_lrfu, bit<16>>(65536) SS;//Regitser

    RegisterAction<data_lrfu, bit<16>, bit<32>>(SS) SS_insert=//SALU
    {
        void apply(inout data_lrfu register_data, out bit<32> result) 
        {
        
        register_data.pure = register_data.pure |+| meta.output5_2;
        register_data.key = meta.output5_1;
        result=register_data.pure;
    ...
    action SS_a()
    {
        SS_insert.execute(hash_7.get({meta.output5_1}));
    }
    @stage(8) table SS_t {
        actions = { SS_a; }
        default_action = SS_a;

    ...
    inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
{
	...
	Register<bit<32>,bit<16>>(65536) counter_count;
    ...
    action check_counter_count(){
		meta.count = counter_count_alu.execute(counter_hash.get({meta.output5_1}));
	}
	...
	Register<bit<32>,bit<16>>(65536) counter_ID;

	RegisterAction<bit<32>,bit<16>,bit<32>>(counter_ID) counter_ID_alu = {
		void apply(inout bit<32> register_data){
			register_data = meta.output5_1;
		}
	};

	action check_counter_ID(){
		counter_ID_alu.execute(counter_hash.get({meta.output5_1}));
	}
